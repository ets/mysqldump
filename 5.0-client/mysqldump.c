...
static struct my_option my_long_options[] =
{
  ...
  {"ignore-table", OPT_IGNORE_TABLE,
   ...
   0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
	{"nullify-field", OPT_NULLIFY_FIELD,
		"Replace all values for the specified field with NULL. To specify more than one field to ignore, "
		"use the directive multiple times, once for each field.  Each field must "
		"be specified with database, table name, and field name, e.g. --nullify-field=database.table.field",
		0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},	
...
#include <sslopt-case.h>
  ...
  case (int) OPT_NULLIFY_FIELD:
	  {
		  if (!strchr(argument, '.'))
		  {
			  fprintf(stderr, "Illegal use of option --nullify-field=<database>.<table>.<field>\n");
			  exit(1);
		  }
		  if (my_hash_insert(&nullify_field, (byte*)my_strdup(argument, MYF(0))))
			  exit(EX_EOM);
		  break;		  
	  }
...
static int get_options(int *argc, char ***argv)
{
  ...
  if (my_hash_insert(&ignore_table,
	...
	if (hash_init(&nullify_field, charset_info, 16, 0, 0,
				  (hash_get_key) get_table_key,
				  (hash_free_key) free_table_ent, 0))
		return(EX_EOM);
...
static void free_resources()
{
  ...
  if (hash_inited(&ignore_table))
	...
	if (hash_inited(&nullify_field))
		hash_free(&nullify_field);	
  if (extended_insert)
...
my_bool include_field(byte* hash_key, uint len)
{
	return !hash_search(&nullify_field, (byte*) hash_key, len);
}
...
static void dump_table(char *table, char *db)
{
  ...
  DBUG_ENTER("dump_table");
    ...
    char hash_key[3*NAME_LEN+4];  /* "db.tablename.fieldname" */	
	char *afterdot;	
	afterdot= strmov(hash_key, db);
	*afterdot++= '.';			
	afterdot = strmov(afterdot, table);
	*afterdot++= '.';					
      ...
      {		  
        ...
        if (!(field= mysql_fetch_field(res)))
          die(EX_CONSCHECK,
                      ...
                      result_table);		  
		  char *end= strmov(afterdot, field->name);		
        ...
        */
        ...
        if (extended_insert && !opt_xml)
        {
          ...
          if (row[i] && include_field(hash_key, (uint) (end - hash_key)))
          {
            if (length)
            {
        ...
        else
        {
		  ...
		  if (row[i] && include_field(hash_key, (uint) (end - hash_key))) //Added check for nullify_field --ets
          {			  
            if (!IS_NUM_FIELD(field))
            {
              if (opt_xml)
...
int main(int argc, char **argv)
{
  ...
  bzero((char*) &nullify_field, sizeof(nullify_field));	
