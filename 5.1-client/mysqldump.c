  ...
  case (int) OPT_NULLIFY_FIELD:
	  {
		  ...
		  if (my_hash_insert(&nullify_field, (uchar*)my_strdup(argument, MYF(0))))
			  exit(EX_EOM);
		  break;
	  }
...
my_bool include_field(uchar* hash_key, uint len)
{
	return !hash_search(&nullify_field, (uchar*) hash_key, len);
}
